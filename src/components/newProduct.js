  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        // Vérifier si la réponse est un tableau, sinon définir un tableau vide
        const productsData = Array.isArray(response.data) ? response.data : [];
        setProducts(productsData);  // Mettre à jour les produits
        setLoading(false);  // Arrêter l'état de chargement
      } catch (err) {
        setError('Erreur lors de la récupération des nouveaux produits');
        setLoading(false);
                ...
                <div className="card">
                  {product.images && product.images.length > 0 ? (
                    ...
                    <img
                      ...
                      src="https://via.placeholder.com/200" // Image de remplacement si aucune image n'est disponible
